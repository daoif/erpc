...
export async function regGetcio(_vartion: Vartion) {
  ...
  if (typeof nvartion !== "number") {
    ...
    console.log("新的生产端URL列表:", _vartion.URL);
    if (_vartion.URL) {
      callURL = _vartion.URL;
      _url = await getcio2(_vartion, callURL);
      if (_url != undefined) {
...
async function recurCIO(
  ...
  rb = await new Promise((res) => {
    ...
    _cioSocket.on("connect", async () => {
      ...
      if (bool) {
        //如果为真,则无需递归,应当返回真
        // console.log("参数验证成功.返回cio");
        _cioSocket.close();
        res(n);
      } else {
        // console.log("参数验证失败,继续递归.");
        _cioSocket.close();
        res(recurCIO(n, callURL, _cioSocket, _vartion));
      }
    });
...
export async function assCioSocket(
  ...
  console.log("assCioSocket _url:", _url);
  if (typeof _url !== "number" && typeof _url !== "undefined") {
    console.log("正式链接ioc");
    let _cioSocket = ioc(_url);
    console.log("_cioSocket.connected:", _cioSocket.connected);
    _enc(_cioSocket);
...
export function on_connect_error(
  ...
  _cioSocket.on("connect_error", async () => {
    console.log("覆盖:链接生产端URL失败:", _url);
    ...
    if (_upIpCycle == 0) {
      ...
      //关闭原链接,避免一直重连.
      _cioSocket.close();
      console.log("将以" + _upIpCycle + "秒为周期尝试重连.");
      setTimeout(
        Rec,
        _upIpCycle * 1000,
        _cioSocket,
...
export async function Rec(
  ...
  if (typeof _url !== "number" && typeof _url !== "undefined") {
    ...
    console.log("将在" + _upIpCycle + "秒后尝试重连.");
    setTimeout(
      Rec,
      _upIpCycle * 1000,
      _cioSocket,
